...
class MainView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["projects"] = Project.objects.filter(is_featured=True).order_by(
            "-date_completed", "-pk"
        )[:3]
...
def contact(request):
    ...
    if not token:
        ...
        return render(
            request, "home/partials/contact_form.html", context_base, status=400
        )
    ...
    try:
        ...
        context_base["recaptcha_error"] = (
            "Could not verify reCAPTCHA right now. Please try again."
        )
        return render(
            request, "home/partials/contact_form.html", context_base, status=502
        )
